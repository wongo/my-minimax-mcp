...
> **なぜ第2のウェブ検索ソースを持つのか？** 多くの web ツール（firecrawl、tavily）はクレジット制・プストン上限制で、枠を使い切るとリサーチが止まります。`minimax_web_search` は MiniMax の月額 Token Plan・独立課金なので、他ツールが上限に達しても動き続けます。返すのはスニペット（全文抽出ではない）なので、firecrawl の 1:1 代替ではなく「主ツールと枠を奪い合わない安定した第2ソース」と捉えてください。
